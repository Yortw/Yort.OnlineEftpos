...
namespace Yort.OnlineEftpos
{
	...
	{
		private OnlineEftposCredentials _Credentials;

		/// <summary>
		/// Default constructor.
		/// </summary>
		/// <param name="credentials">A <see cref="OnlineEftposCredentials"/> instance containing the credentials used to obtain tokens from the Online Eftpos API.</param>
		public OnlineEftposCredentialsProvider(OnlineEftposCredentials credentials)
		{
			credentials.GuardNull(nameof(credentials));
		...
		{
			return new OnlineEftposCredentials(CopyString(_Credentials.ConsumerKey), CopyString(_Credentials.ConsumerSecret));
		}
		...
		private static string CopyString(string data)
		{
